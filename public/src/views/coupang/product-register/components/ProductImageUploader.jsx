...
const ProductImageUploader = ({ productId }) => {
  ...
  const { products, updateProduct } = useCoupangStore();
  ...
  const images = product.additionalImages ? product.additionalImages : [];

  const handleImageUpload = async (files) => {
    if (!files || files.length === 0) return;
    
    if (!isFirstProduct && files.length > 1) {
      alert('대표 이미지 1개만 업로드할 수 있습니다.');
      return;
    }

    const filesToProcess = Array.from(files).slice(0, isFirstProduct ? 10 - images.length : 1);
    if (filesToProcess.length === 0) {
      alert(isFirstProduct ? '상품 이미지는 최대 10개까지 업로드할 수 있습니다.' : '이미지를 업로드할 수 없습니다.');
      return;
    }
    ...
    for (const [idx, file] of filesToProcess.entries()) {
      ...
      try {
        ...
        if (isFirstProduct) {
          ...
          // 대표 이미지 업데이트
          updateProduct(productId, 'additionalImages', images);
        }
      } catch (error) {
    ...
    if (newImages.length > 0 && isFirstProduct) {
      updateProduct(productId, 'additionalImages', [...images, ...newImages]);
      setDragActive(false);
    }
  };
  ...
  const handleImageRemove = (imageId) => {
    ...
    updateProduct(productId, 'additionalImages', reorderedImages);
  ...
  return (
    <CCard className="mb-4">
      <CCardHeader>
        <strong>{isFirstProduct ? '상품 이미지' : '대표 이미지'}</strong>
      </CCardHeader>
      <CCardBody>
        {images.length === 0 ? (
          <div 
          ...
          >
            {isFirstProduct ? (
              ...
              <div 
              className={`detail-image-item ${selectedImage ? 'selected' : ''} ${images[0]?.isMain ? 'main-image' : ''}`}
              onClick={() => handleImageSelect(images[0]?.id)}
              onDoubleClick={() => handleImageRemove(images[0]?.id)}>
                <CImage rounded thumbnail src={images[0]?.preview} width={130} height={130} alt={images[0]?.name} draggable="false" />
                <div className="main-image-badge">대표</div>
              </div>
            )}
            {((isFirstProduct && images.length < 10) || (!isFirstProduct && images.length < 1)) && (
